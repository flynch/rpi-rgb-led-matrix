...
#cgo LDFLAGS: -lrgbmatrix -L${SRCDIR}/../../../lib -lstdc++ -lm
...
func (c *HardwareConfig) geometry() (width, height int) {
	if c.PixelMapping == "V-mapper" {
		return c.Cols * c.Parallel, c.Rows * c.ChainLength
	} else {
...
func NewRGBLedMatrix(config *HardwareConfig) (c Matrix, err error) {
	...
	w, h := config.geometry()
	fmt.Printf("Creating led_matrix_create_from_options with width: %d height: %d\n", w, h)
	m := C.led_matrix_create_from_options(config.toC(), nil, nil)
	fmt.Printf("Creating offscreen canvas \n")
	...
	fmt.Printf("created RGBLedMatrix \n")
